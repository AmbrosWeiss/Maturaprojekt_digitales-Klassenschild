...
#include "GxGDEW075Z09_modifiziert.h"
...
/*
Beginn: eingefügter Teil Ambros Weiß HTBLA Braunau____________________________________________________________________________________________
*/
void GxGDEW075Z09::BitmapToBuffer(const uint8_t* bitmap, uint32_t size)
{
    _using_partial_mode = false; // remember
    _wakeUp();
    IO.writeCommandTransaction(0x10);
    for (uint32_t i = 0; i <= GxGDEW075Z09_BYTE_SIZE; i++)
    {
      uint64_t data = 0x00; // white is 0x00 on device
      ...
      if (i < size)
      {
          ...
          #else
          data = bitmap[i];
        //_black_buffer[i] = bitmap[i];
        #endif

      //if (mode & bm_invert) data = ~data;
      }
      _black_buffer[i] = data;
    }
    //IO.writeCommandTransaction(0x12);      //display refresh
    _waitWhileBusy("BitmapToBuffer");
    _sleep();

}
/*
Ende: von mir eingefügter Teil______________________________________________________________________________________________
*/
